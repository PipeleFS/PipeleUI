...
<script>
    ...
    import { getData, getSharedofOwnerQuery } from "./queries.js";
    ...
    $: listItems = getData(getSharedofOwnerQuery, signerAddress);
...
{:then list}
    {#each list.data.data.pipeleEntities as item, index}
        <ul>
            <li>
                <input id={`item-${index}`} type="checkbox" bind:group={$selectedShares} value="{item.key}" />
